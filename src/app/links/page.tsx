...
export default function LinksPage() {
  ...
  const phoneNumberOnly = firstPhone ? firstPhone.number.replace(/-/g, '') : CONTACT.phone_Amir.split(' ')[0].replace(/-/g, '');
  const email = settings.contact.email || CONTACT.email;
  const facebook = settings.contact.socials.facebook || "https://www.facebook.com/MSSPSU";
  const instagram = settings.contact.socials.instagram || "https://www.instagram.com/msspsuhatyai/";
  const tiktok = settings.contact.socials.tiktok || "https://www.tiktok.com/@msspsuhatyai";
  const youtube = settings.contact.socials.youtube || "https://www.youtube.com/@msspsuhatyai";
  const line = settings.contact.socials.line || "https://line.me/R/ti/p/@mss-psu";
 ...
 return (
 <main className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-100 to-blue-200 flex flex-col items-center py-12 px-4 sm:px-6 relative overflow-hidden font-sans transition-colors duration-500">
 ...
 <button 
 onClick={handleShare}
 className="absolute top-0 right-0 p-2.5 bg-white/60 backdrop-blur-md rounded-full shadow-sm border border-white/60 text-gray-600 hover:bg-white transition-all z-20"
 aria-label="Share"
 >
 ...
 <div className="absolute -inset-1 bg-gradient-to-r from-blue-400 to-sky-300 rounded-full blur opacity-40 group-hover:opacity-70 transition duration-500"></div>
 
 <div className="relative w-28 h-28 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center p-3 shadow-xl border border-white/50 ">
 <Image
 ...
 <h1 className="text-2xl font-bold text-blue-950 text-center mb-1 drop-shadow-sm">
 ชมรมมุสลิม ม.อ.หาดใหญ่
 </h1>
 <p className="text-blue-700/80 text-sm text-center mb-4 font-medium">
 Muslim Student Society · PSU Hat Yai
 </p>

 <div className="inline-flex items-center gap-2 bg-white/60 backdrop-blur-md border border-white/60 rounded-full px-4 py-1.5 shadow-sm">
 <span className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.8)] animate-pulse"></span>
 <span className="text-sm font-medium text-blue-900 ">หวังดีดี จากบ้านหลังเดิม</span>
 </div>
 </motion.div>
 ...
 key={link.id}
 onClick={link.onClick}
 className={`${baseClasses} bg-white/80 backdrop-blur-md border border-white hover:border-blue-200 hover:bg-white shadow-[0_4px_12px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_20px_-6px_rgba(59,130,246,0.2)] hover:-translate-y-1 text-left`}
 variants={itemVariants}
 >
 {content}
 </motion.button>
 ...
 className={`${baseClasses} bg-white/80 backdrop-blur-md border border-white hover:border-blue-200 hover:bg-white shadow-[0_4px_12px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_20px_-6px_rgba(59,130,246,0.2)] hover:-translate-y-1`}
 variants={itemVariants}
 >
 {content}
 </motion.a>
 ...
 className="group flex items-center gap-3 w-full p-3 mt-4 rounded-xl border-2 border-dashed border-blue-300 hover:border-blue-400 hover:bg-blue-50/50 transition-all duration-300"
 variants={itemVariants}
 >
 ...
 <div>
 <div className="text-sm font-semibold text-blue-900 ">กลับสู่หน้าหลัก</div>
 <div className="text-xs text-blue-600/70 ">ไปที่เว็บไซต์ msspsuhatyai.org</div>
 </div>
 </motion.a>
 </motion.div>
 ...
 <button 
 className="absolute top-4 right-4 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200 hover:text-gray-800 transition-colors z-10" 
 onClick={() => setShowDonateModal(false)}
 aria-label="Close"
 >
 ...
 <h3 className="text-xl font-bold text-blue-900 ">สนับสนุนการทำงานของชมรม</h3>
 <p className="text-sm text-gray-500 mt-1">ร่วมเป็นส่วนหนึ่งในการขับเคลื่อนกิจกรรม</p>
 </div>

 <div className="flex flex-col items-center relative z-10">
 <div className="relative w-[200px] h-[200px] mb-5 p-2 bg-white rounded-xl shadow-md border border-blue-100 ">
 <Image
 src={qrImage}
 alt="QR พร้อมเพย์"
 fill
 ...
 <div className="w-full space-y-2.5 bg-blue-50/50 p-4 rounded-xl border border-blue-100/50 ">
 <div className="flex flex-col items-center gap-1">
 <span className="text-xs text-gray-500 ">ชื่อบัญชี</span>
 <span className="font-semibold text-gray-900 text-sm">{accountName}</span>
 </div>
 
 <div className="h-px w-full bg-gradient-to-r from-transparent via-blue-200 to-transparent my-1"></div>
 
 <div className="flex flex-col items-center gap-1">
 <span className="text-xs text-gray-500 ">ธนาคาร</span>
 <span className="font-medium text-gray-800 text-sm">{bank}</span>
 </div>
 ...
 <span className="font-bold text-lg text-blue-700 tracking-wider">{accountNumber}</span>
 <button
 onClick={copyAccountNumber}
 className="p-1.5 rounded-lg bg-blue-100 text-blue-600 hover:bg-blue-200 hover:text-blue-800 transition-colors"
 ...
 animate={{ opacity: 1, y: 0 }}
 className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-gray-800/90 backdrop-blur-sm text-white text-xs px-4 py-2 rounded-full shadow-lg"
 >
 คัดลอกเลขบัญชีแล้ว
 </motion.div>
 )}
