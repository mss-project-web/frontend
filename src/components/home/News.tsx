...
import { useActivitiesThisMonth } from "@/hooks/activities/useActivitiesThisMonth";

function NewsCardSkeleton() {
  return (
    <div className="rounded-lg border border-blue-100 bg-blue-50 shadow-sm transition flex flex-col h-full">
      <div className="relative w-full aspect-[4/3] rounded-t-lg overflow-hidden bg-blue-100 shrink-0">
...
export function NewsAndEvents() {
  ...
  return (
    // <--- This 'return' statement should be correctly positioned
      ...
      <div className="grid md:grid-cols-3 gap-10">
        ...
        <div className="md:col-span-2">
          ...
          <div className={`grid gap-3 sm:gap-4 md:gap-6 ${
            newsLoading || newsError ? 'grid-cols-2 md:grid-cols-3' : 
            news.length === 1 ? 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3' : 
            ...
            {newsLoading ? (
              ...
              news.map((newsItem) => (
                <motion.div
                  key={newsItem._id}
                  whileHover={{ y: -4 }}
                  className="rounded-lg border border-blue-100 bg-blue-50 shadow-sm hover:shadow-md transition flex flex-col h-full overflow-hidden"
                >
                  <a
                    href={newsItem.link}
                    target="_blank"
                  ...
                  >
                    ...
                    <div className="p-4 flex-grow flex flex-col justify-between">
                      <div>
                        <h4 className="font-semibold text-[14px] md:text-base text-blue-700 mb-1.5 line-clamp-2 leading-snug">
                          {newsItem.name}
                        </h4>
                        <div className="text-xs md:text-sm text-blue-700/80 line-clamp-2 md:line-clamp-3 mb-3 leading-relaxed">
                          {newsItem.description}
                        </div>
                      </div>
                      <p className="text-[10px] md:text-xs text-blue-800/70 font-medium">
                        โพสต์เมื่อ{" "}
